export class BoggleGame {
    ...
    $('#word').on('input', ()=> {
        //console.log('Input value changed:', $('#word').val());
        this.onCurrentWordChanged($('#word').val())
    });
    ...
    onGameJoined(data) {
        ...
        this.updateUI(data);
        this.countdownTimer(this.gameData.currentTime, $('#timer'));
        $('#word').focus();
    ...
    onWordGuessed(data) {
        ...
        $(listItem).hover(
            () => {
                this.onCurrentWordChanged(data.word);
            },
            () => {
    ...
    countdownTimer(duration, display) {
        ...
        this.timerId = setInterval(() =>{
            ...
            if (--timer < 0) {
                this.updateTimerColor(timer, display);
                clearInterval(this.timerId);
            } else {
                this.updateTimerColor(timer, display);
            }
    ...
    updateUI(data) {
        ...
        if (data.currentState != prevState) { // On state changed to started
            this.onStateChanged(prevState, data.currentState);
        }
    ...
    onStateChanged(prevState, newState) {
        ...
        // Timer
        console.log("Restarting timer for : " + this.gameData.currentTime + " seconds")
        this.countdownTimer(this.gameData.currentTime, $('#timer'));
        ...
        if (newState === "STARTED") {
            ...
            resultWords.forEach((word, index) => {
                $(word).hover(
                    () => {
                        ...
                        this.onCurrentWordChanged(parsedWord);
                    },
                    () => {
                        this.onCurrentWordChanged("");
                    }
            ...
            this.showGameMessage("Une nouvelle partie va bientôt commencer...");
        }
    }
    ...
    sendWord() {
        ...
        const payload = JSON.stringify(data)
        this.ws.send("");//TODO : check if necessary
    ...
    onCurrentWordChanged(word) {
        ...
        const path = this.findWordPath(grid2D, word.toLowerCase());
        grid.forEach((letter, index) => {
            $("#" + letter.id).removeClass("game-grid-item-highlighted");
